...
//                }
 ...
 /*      
    ...
    function get_product_list(){
       ...
       /* $this->db->select('warehouses_products_variants.id,warehouses_products_variants.quantity,products.id as product_id,products.name,products.code,variants.name as veriants_name')
                ->join('products','products.id = warehouses_products_variants.product_id','left')
                ->join('variants','variants.id = warehouses_products_variants.option_id','left')
                ->where(['warehouses_products_variants.warehouse_id'=>$get_arg[0]])->order_by('products.name','ASC');
        $get_data = $this->db->get('warehouses_products_variants')->result();
        return  $get_data;*/
         
        
       /* "SELECT p.`id`, p.`code`, p.`name`, wp.warehouse_id, wp.quantity 
            FROM  `sma_products` p  LEFT OUTER JOIN  `sma_warehouses_products` wp  on   p.`id`=  wp.product_id  
            and  wp.warehouse_id = 2"*/
