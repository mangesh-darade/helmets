...
class Customers extends MY_Controller
{
  ...
  */
   $this->datatables
          ->select("id, company, name, email, phone, price_group_name, customer_group_name, gstn_no, deposit_amount");
	...
	$this->form_validation->set_rules('phone', lang("phone"), 'is_unique[companies.phone]');
        ...
        if ($this->form_validation->run('customer/add') == true) {
            ...
            if($this->uri->segment(3) == 'quick') {
                $synch_customer_data = false;
                list($customer_group_id,  $customer_group_name )  = explode('~',$this->input->post('customer_group'));                
                list($price_group_id,  $price_group_name ) = $this->input->post('price_group') ? explode('~', $this->input->post('price_group')) : '';            
                list($state,  $state_code )  = ($this->input->post('state')) ? explode('~',$this->input->post('state')) :'';
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                   ...
                   $style = array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,),'font' => array('name' => 'Arial', 'color' => array('rgb' => 'FF0000')), 'borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_NONE, 'color' => array('rgb' => 'FF0000') )));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('M2', lang('ccf1'));
                    $this->excel->getActiveSheet()->SetCellValue('N2', lang('ccf2'));
                    $this->excel->getActiveSheet()->SetCellValue('O2', lang('ccf3'));
                    $this->excel->getActiveSheet()->SetCellValue('P2', lang('ccf4'));
                    $this->excel->getActiveSheet()->SetCellValue('Q2', lang('ccf5'));
                    $this->excel->getActiveSheet()->SetCellValue('R2', lang('ccf6'));
