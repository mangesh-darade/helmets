            ...
            if(is_array($upOrders)){
                ...
                $tablelist .='<tr>
                        ...
                        $tablelist .='  </td>
                            ...
                            } else { $tablelist .='<button class="btn btn-xs default">Not Assign</button>'; } 
        ...
        echo '<div>
            ...
            foreach($up_response->order->items as $items) {       
                ...
                echo ' <tr>
                           ...
                           <td>'.(!empty($items->options_to_add) ? join(',', $optionAddName)  : '').'</td>
                           <td>'.(!empty($items->options_to_remove) ? join(',', $optionRemoveName)  : '').'</td>
             ...
             case 'Single_Product':
	             ...
	             if(is_array($taxarray)){
	                 $pass['taxes']=$taxarray; 
	                    ...
	                    $URL = $this->upApiUrl . 'external/api/v1/inventory/locations/-1/';
   ...
   /*========================================================================
    ...
    public function getstore(){
        ...
        $tabledata = '';
            $tabledata.='<table id="storelist" class="table table-bordered table-hover table-striped">';
				...
				$DeleteAction = "<a href='javascript:void(0);' ".$Onclick." class='tip po delete_up_store' > <i class='fa fa-trash-o'></i> Delete</a>";
                                }
                            $tabledata.='</td>';
                            $tabledata.='<td class="text-center"><a href="'.site_url('urban_piper/update_store/').$store->id.'" data-toggle="modal" data-target="#myModal2" class="btn btn-small btn-info" > <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a> '.$DeleteAction.' </td>';
                        $tabledata.='</tr>';
                        $sr++; 
                    }
                $tabledata.= '</tbody>';
    ...
    public function add_store(){
        if($this->input->post()){
              ...
              $this->form_validation->set_rules('notification_phones', 'Notification Phone No.', 'trim|required|numeric|exact_length[10]');
              $this->form_validation->set_rules('notification_emails', 'Notification Email Id', 'trim|required|valid_email');
