...
class Eshop_sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = NULL)
    {
       ...
       // $email_link = anchor('sales/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_sale'), 'data-toggle="modal" data-target="#myModal"');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no,".$this->db->dbprefix('sales') . ".id as invoice_id, biller, customer, (grand_total+IFNULL(`rounding`, 0)) , paid, (grand_total-paid) as balance, sale_status,payment_status,delivery_status")
                ->from('sales')
                ->where('eshop_sale', 1);
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no,".$this->db->dbprefix('sales') . ".id as invoice_id, biller, customer, (grand_total+IFNULL(`rounding`, 0))  s, paid, (grand_total+IFNULL(`rounding`, 0)-paid) as balance, sale_status,payment_status,delivery_status")
                ->from('sales')
                ->where('eshop_sale', 1);
        }
