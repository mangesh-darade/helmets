	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(sortedItems, function () {
			...
			var item_pending_qty = item.row.pending_qty;
                        var product_unit = item.row.unit, base_quantity = (parseInt(item.row.base_quantity)) ? item.row.base_quantity : item.row.qty;
						/*03-10-2019*/
			...
			if(sale_action=='add'){
				...
				var show_net_price = formatMoney(parseFloat(item_price) * parseFloat(base_quantity));
			}else{
			...
			//tr_html += '<td class="delivery_items"><input class="form-control text-center rpending_quantity" tabindex="" name="pending_quantity[]" type="number" value="' + formatDecimal(item_pending_qty) + '" min="0" max="' + formatDecimal(item_qty) + '"  data-id="' + row_no + '" data-item="' + item_id + '" id="pending_quantity_' + row_no + '" onchange="validate_qty(this);" onClick="this.select();"></td>';
                        ...
                        tr_html +='<td class="text-right">'+show_net_price +' </td>';
			total_netprice += parseFloat(show_unit_price) * parseFloat(base_quantity);//item_price
...
function add_invoice_item(item) {
	...
	if (slitems[item_id]) {
		slitems[item_id].row.qty = parseFloat(slitems[item_id].row.qty) + 1;
                slitems[item_id].row.base_quantity = parseFloat(slitems[item_id].row.base_quantity ) + 1;
	} else {
